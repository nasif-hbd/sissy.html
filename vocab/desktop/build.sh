...
#
#   ./build.sh                 → VocabX.exe beside this script
set -euo pipefail
cd "$(dirname "$0")"
...
"$CC" -O2 -s -o VocabX.exe vocabx.c -lws2_32 -lshell32 -mwindows
echo "built $(ls -lh VocabX.exe | awk '{print $5}')  →  $(pwd)/VocabX.exe"
